...
class VentaService {
  static Future<List<Compra>> obtenerVentas() async {
    ...
    final response = await http.get(Uri.parse(ApiConstants.ventas), headers: headers);
  ...
  static Future<List<Compra>> obtenerVentasDelVendedor() async {
    final sesion = await SessionService.obtenerSesion();
    final nombreVendedor = sesion?['nombre'] ?? '';
    final headers = await SessionService.headersConToken();
    final uri = Uri.parse(ApiConstants.ventas).replace(queryParameters: {'vendedor': nombreVendedor});
  ...
  static Future<List<Compra>> obtenerMisCompras() async {
    ...
    final headers = await SessionService.headersConToken();
    final uri = Uri.parse(ApiConstants.ventas).replace(queryParameters: {'cliente': nombreCliente});
    final response = await http.get(uri, headers: headers);
    if (response.statusCode == 200) {
      final List data = jsonDecode(response.body);
      return data.map((e) => Compra.fromJson(e)).toList();
  ...
  static Future<Compra> registrarVenta({
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      final ventaData = data['venta'] ?? data;
      return Compra.fromJson(ventaData);
    }
